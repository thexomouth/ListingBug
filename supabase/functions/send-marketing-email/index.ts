...
Deno.serve(async (req: Request) => {
  ...
  const { recipients, subject, body: emailBody, campaign_name, sender_id, attachments, unsubscribe_url, campaign_id: providedCampaignId } = body;
  ...
  // Use provided campaign_id (pre-generated by client) or generate a new one
  const campaignId = providedCampaignId ?? crypto.randomUUID();

  // Load global suppression list and campaign-specific suppressions, filter both
  const [{ data: suppressedRows }, { data: campaignSuppressedRows }] = await Promise.all([
    serviceClient.from('suppression_list').select('email').eq('user_id', user.id),
    serviceClient.from('campaign_suppressions').select('email').eq('user_id', user.id).eq('campaign_id', campaignId),
  ]);
  const suppressedEmails = new Set([
    ...(suppressedRows ?? []).map((r: any) => r.email.toLowerCase()),
    ...(campaignSuppressedRows ?? []).map((r: any) => r.email.toLowerCase()),
  ]);
  ...
  for (const recipient of sendableRecipients) {
    ...
    const unsubscribeHref = (() => {
      try {
        ...
        return `${unsubscribe_url}?email=${encodeURIComponent(recipient.email)}`;
      }
    })();
    const unsubscribeFooter = `<br><br><hr style="border:none;border-top:1px solid #eee;margin:32px 0"><p style="font-size:11px;color:#aaa;text-align:center;margin:0;line-height:1.7">You are receiving this email because your contact information appears in a public real estate listing.<br>This message was sent by a ListingBug user. &nbsp;·&nbsp; <a href="${unsubscribeHref}" style="color:#aaa;text-decoration:underline">Unsubscribe</a> &nbsp;·&nbsp; <a href="https://www.thelistingbug.com" style="color:#aaa;text-decoration:none">ListingBug</a></p>`;
