              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-lg text-[#000000]">Field Mapping</h3>
                  </div>
                  <Tooltip content="We automatically map your fields to the destination format">
                    <HelpCircle className="w-4 h-4 text-gray-400 cursor-help" />
                  </Tooltip>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                  <p className="text-sm text-blue-900">
                    <strong>Auto-mapping complete!</strong> We've analyzed your data and {selectedIntegration?.name}'s requirements to suggest the best field mappings.
                  </p>
                </div>

                {/* Field Mappings Table */}
                <div className="border border-gray-300 rounded-lg overflow-hidden">
                  <div className="bg-gray-100 border-b border-gray-300 px-4 py-3 grid grid-cols-11 gap-3 text-xs font-bold text-gray-700 uppercase">
                  ...
                  <div className="divide-y divide-gray-200 bg-white max-h-[400px] overflow-y-auto">
                    {fieldMappings.map((mapping) => (
                      <div key={mapping.source} className="px-4 py-3 grid grid-cols-11 gap-3 items-center hover:bg-gray-50">
                        <div className="col-span-5">
                          <span className="text-sm font-bold text-gray-900">{mapping.source}</span>
                          {mapping.required && (
                            <span className="ml-2 text-xs text-red-600 font-bold">*</span>
                          )}
                        </div>
                        <div className="col-span-1 text-center text-gray-400">→</div>
                        <div className="col-span-5 text-sm text-gray-700 font-medium">{mapping.destination}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-300 rounded-lg p-3">
                  <p className="text-xs text-amber-900">
                    <strong>* Required fields</strong> - These fields must have data for successful sync
                  </p>
                </div>

                <LBButton
                  variant="primary"
                  onClick={() => setMappingsAccepted(true)}
                  disabled={mappingsAccepted}
                  className="w-full"
                >
                  <Sparkles className="w-4 h-4 mr-2" />
                  {mappingsAccepted ? '✓ Mappings Accepted' : 'Accept All Mappings'}
                </LBButton>
              </>
            )}
