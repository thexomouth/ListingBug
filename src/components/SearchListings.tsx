...
export function SearchListings({ onAddToMyReports, onNavigate, onViewSearchResults }: SearchListingsProps = {}) {
  ...
  const handleSaveSearch = () => {
    // Prefill with city + state so user can just press Save if satisfied
                    ...
                    <CardContent className="pt-6">
                      ...
                      <div className="flex items-center justify-end pt-3 border-t border-gray-100 dark:border-white/10">
                        <div className="flex items-center gap-2">
                          ...
                          <LBButton variant="primary" size="sm" onClick={() => { if (onNavigate) { sessionStorage.setItem('listingbug_automations_tab', 'create'); onNavigate('automations'); } }}><Zap className="w-3.5 h-3.5 mr-1.5" />Automate</LBButton>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
        ...
        <div className="space-y-4 pb-[100px]">
          {searchHistory.length === 0 ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between"><h3 className="font-bold text-[24px]">Recent Searches ({searchHistory.length})</h3><LBButton variant="outline" size="sm" onClick={() => { setSearchHistory([]); toast.success('Search history cleared'); }}><Trash2 className="w-3.5 h-3.5 mr-1.5" />Clear</LBButton></div>
