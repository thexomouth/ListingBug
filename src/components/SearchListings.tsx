...
export function SearchListings({ onAddToMyReports, onNavigate }: SearchListingsProps = {}) {
  ...
  const handleSearch = () => {
    // Walkthrough: Mark search as performed for Step 4 trigger
    setSearchPerformed(true);
    
    // Scroll to top of page before showing loading modal
    window.scrollTo({ top: 0, behavior: 'instant' });
    
    // Lock scrolling when loading starts (also handled by useEffect above)
    document.body.style.overflow = 'hidden';
    ...
    setTimeout(() => {
      // Mock search results with enhanced data - Generate 72 results
      const cities = ['Los Angeles', 'San Diego', 'Santa Monica', 'Pasadena', 'Long Beach', 'Irvine', 'Anaheim', 'Riverside'];
      const propertyTypes = ['Single Family', 'Condo', 'Townhouse', 'Multi-Family'];
      const statuses = ['Active', 'Pending', 'Active', 'Active']; // More active listings
      const agentNames = [
        'Sarah Johnson', 'Michael Chen', 'Jennifer Martinez', 'David Thompson', 
        'Amanda Wilson', 'Robert Garcia', 'Lisa Anderson', 'James Brown',
        'Maria Rodriguez', 'John Davis', 'Emily White', 'Daniel Lee'
      ];
      const brokerages = [
        'Premier Realty Group', 'Luxury Properties Inc', 'Home Experts Realty',
        'Coastal Realty Partners', 'Beachfront Properties', 'Elite Real Estate',
        'Metro Realty', 'Prime Locations Realty'
      ];
      
      // Curated array of real house photos from Unsplash
      const housePhotos = [
        ...
        'https://images.unsplash.com/photo-1572120360610-d971b9d7767c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1601760562234-9814eea6663a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1583608205776-bfd35f0d9f83?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        ...
        'https://images.unsplash.com/photo-1592595896551-12b371d546d5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1570129477492-45c003edd2be?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1575517111478-7f6afd0973db?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1523217582562-09d0def993a6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1582268611958-ebfd161ef9cf?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1598228723793-52759bba239c?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1599809275671-b5942cabc7a2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1613553507747-5f8d62ad5904?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1593696954816-0c99e7f2f4fd?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1576941089067-2de3c901e126?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        ...
        'https://images.unsplash.com/photo-1605276373954-0c4a0dac5b12?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1558036117-15d82a90b9b1?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1571055107559-3e67626fa8be?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080',
        'https://images.unsplash.com/photo-1577495508326-19a1b3cf65b7?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080'
      ];
      
      const mockResults = [];
      
      for (let i = 1; i <= 72; i++) {
        const hasPriceDrop = Math.random() > 0.7;
        const hasReList = Math.random() > 0.8;
        const daysListed = Math.floor(Math.random() * 45) + 1;
        const priceBase = 400000 + Math.floor(Math.random() * 1500000);
        const bedrooms = Math.floor(Math.random() * 4) + 2;
        const bathrooms = Math.floor(Math.random() * 3) + 1;
        
        mockResults.push({
          id: i,
          address: `${100 + i * 10} ${['Main', 'Oak', 'Pine', 'Elm', 'Maple', 'Cedar', 'Birch', 'Willow'][i % 8]} ${['St', 'Ave', 'Rd', 'Blvd', 'Way', 'Dr', 'Ln', 'Ct'][i % 8]}`,
          city: cities[i % cities.length],
          state: 'CA',
          zip: `9${String(i).padStart(4, '0')}`,
          propertyType: propertyTypes[i % propertyTypes.length],
          bedrooms: bedrooms,
          bathrooms: bathrooms + (Math.random() > 0.5 ? 0.5 : 0),
          sqft: 1000 + Math.floor(Math.random() * 2500),
          lotSize: propertyTypes[i % propertyTypes.length] === 'Condo' ? 0 : 3000 + Math.floor(Math.random() * 7000),
          yearBuilt: 1985 + Math.floor(Math.random() * 38),
          status: statuses[i % statuses.length],
          price: priceBase,
          daysListed: daysListed,
          agentName: agentNames[i % agentNames.length],
          agentPhone: `(555) ${String(Math.floor(Math.random() * 900) + 100)}-${String(Math.floor(Math.random() * 9000) + 1000)}`,
          agentEmail: `${agentNames[i % agentNames.length].toLowerCase().replace(' ', '.')}@realty.com`,
          brokerage: brokerages[i % brokerages.length],
          reList: hasReList,
          priceDrop: hasPriceDrop,
          priceDropAmount: hasPriceDrop ? Math.floor(Math.random() * 75000) + 10000 : 0,
          priceDropPercent: hasPriceDrop ? (Math.random() * 8 + 1).toFixed(1) : 0,
          latitude: 33.5 + Math.random() * 1.5,
          longitude: -118.5 + Math.random() * 1.5,
          description: `${propertyTypes[i % propertyTypes.length]} in ${cities[i % cities.length]}. ${bedrooms} bed, ${bathrooms} bath. ${hasPriceDrop ? 'Price reduced!' : 'Great opportunity!'} ${hasReList ? 'Recently re-listed.' : ''}`,
          photos: [housePhotos[i % housePhotos.length]],
          mlsNumber: `CA${String(i).padStart(8, '0')}`,
          mlsSource: 'CRMLS',
        });
      }
      ...
      // Update listings synced count
      setListingsSynced(prev => Math.min(prev + mockResults.length, listingsCap));
      
      // Unlock scrolling when loading ends
      document.body.style.overflow = 'unset';
      ...
      setSearchHistory(prev => [historyEntry, ...prev.slice(0, 49)]); // Keep last 50 searches
