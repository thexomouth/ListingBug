...
// ---------------------------------------------------------------------------
// Types (identical to V2Dashboard)
// ---------------------------------------------------------------------------
...
export function V2DashboardV2() {
  ...
  useEffect(() => {
    const claimPending = async () => {
      ...
      try {
        ...
        await supabase.functions.invoke('send-campaign-emails', { body: { campaign_id: campaign.id } });
  ...
  const handleReconnect = async (provider: string) => {
    try {
      ...
      console.error('[V2DashboardV2] Reconnect failed:', err);
    }
  };
