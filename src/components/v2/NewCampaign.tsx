...
export function NewCampaign() {
  ...
  const renderMailboxStep = () => {
    const gmailSender = senders.find(s => s.integration_id === 'gmail') ?? null;
    const outlookSender = senders.find(s => s.integration_id === 'outlook') ?? null;
    const smtpSender = senders.find(s => s.integration_id === 'smtp') ?? null;

    const isGmailSelected = !!gmailSender && selectedSender === gmailSender.id;
    const isOutlookSelected = !!outlookSender && selectedSender === outlookSender.id;
    const isSmtpSelected = !!smtpSender && selectedSender === smtpSender.id;

    return (
      <div className="mb-2">
        <div className="text-base font-medium text-gray-900 dark:text-white mb-1">Which mailbox would you like to use?</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 truncate">
                  {gmailSender ? gmailSender.from_email : 'Send via Google OAuth'}
                </div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 truncate">
                  {outlookSender ? outlookSender.from_email : 'Send via Microsoft OAuth'}
                </div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 truncate">
                  {smtpSender ? smtpSender.from_email : 'Use your own mail server (SendGrid, Mailchimp, or any SMTP provider)'}
                </div>
  ...
  const renderStep0Confirm = () => (
    <div className="mb-2">
      ...
      <div className="space-y-2 mb-5">
        {[
          { label: 'From Name', value: businessInfo.business_name },
  ...
  const renderStep3 = () => {
    ...
    return (
      <div>
        {/* Header */}
        <div className="mb-6">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-1">Review and go live</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-6">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full py-3 rounded-lg text-sm font-bold transition-opacity disabled:opacity-60 hover:opacity-90 shadow-sm"
              style={{ background: '#FFCE0A', color: '#342e37' }}
            >
              Send first emails →
            </button>
