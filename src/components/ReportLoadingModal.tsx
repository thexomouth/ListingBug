...
export function ReportLoadingModal({ isOpen, reportName, onCancel }: ReportLoadingModalProps) {
  const loadingSteps = [
    ...
    <Sheet open={isOpen} onOpenChange={onCancel}>
      <SheetContent className="w-full sm:max-w-2xl lg:max-w-4xl p-0 flex flex-col overflow-hidden">
        <SheetHeader className="px-6 pt-6 pb-4 border-b bg-[#ffd447]">
          <SheetTitle className="text-2xl text-[#342e37]">Generating Report</SheetTitle>
          <SheetDescription className="text-[15px] text-[#342e37]/80">
            Fetching listing data for {reportName}
          </SheetDescription>
        </SheetHeader>
