...
export function ListingDetailModal({ listing, onClose, onSaveListing, isSaved = false }: ListingDetailModalProps) {
  ...
  // Mock property record data — all fields null-guarded so this never crashes
  const generatePropertyRecord = () => {
    const baseYear = listing.yearBuilt ?? 2000;
    const basePrice = listing.price ?? 300000;
    const cityName = (listing.city ?? 'UNKNOWN').toUpperCase();
    const propType = listing.propertyType ?? '';
    const purchaseYear = baseYear + Math.floor(Math.random() * 10);
    const purchasePrice = Math.floor(basePrice * (0.6 + Math.random() * 0.2));
    const taxAssessment = Math.floor(basePrice * 0.85);
    ...
    return {
      ...
      annualTax,
      legalDescription: `LOT 12, BLOCK 5, ${cityName} HEIGHTS SUBDIVISION, ACCORDING TO THE PLAT THEREOF RECORDED IN VOLUME 47 OF PLATS, PAGE 23`,
      parcelNumber: `${Math.floor(Math.random() * 900 + 100)}-${Math.floor(Math.random() * 900 + 100)}-${Math.floor(Math.random() * 9000 + 1000)}`,
      zoning: propType === 'Commercial' ? 'C-2 (General Commercial)' : propType === 'Multi-Family' ? 'R-3 (Multi-Family Residential)' : 'R-1 (Single Family Residential)',
      deedType: 'Warranty Deed',
      recordedDate: `${purchaseYear}-04-02`,
      liens: Math.random() > 0.7 ? [
        { type: 'Mortgage Lien', amount: Math.floor(basePrice * 0.7), holder: 'First National Bank', recorded: `${purchaseYear}-04-02` }
      ] : [],
      permits: [
      ...
      salesHistory: [
        ...
        { date: `${baseYear}-01-10`, price: Math.floor(purchasePrice * 0.45), type: 'New Construction' },
      ],
    };
  };

  // Mock valuation data — all fields null-guarded so this never crashes
  const generateValuation = () => {
    const basePrice = listing.price ?? 300000;
    const baseSqft = (listing.sqft && listing.sqft > 0) ? listing.sqft : 1500;
    const estimatedValue = Math.floor(basePrice * (0.95 + Math.random() * 0.1));
    const lowRange = Math.floor(estimatedValue * 0.92);
    const highRange = Math.floor(estimatedValue * 1.08);
    const pricePerSqFt = Math.floor(estimatedValue / baseSqft);
    const rentEstimate = Math.floor(estimatedValue * 0.008);

    const comps = [
      { address: '1456 Maple Ave', distance: '0.3 mi', price: Math.floor(estimatedValue * 0.98), beds: listing.bedrooms ?? 3, baths: listing.bathrooms ?? 2, sqft: baseSqft + Math.floor(Math.random() * 200 - 100), soldDate: '2024-11-15', daysOnMarket: 12 },
      { address: '892 Birch Street', distance: '0.5 mi', price: Math.floor(estimatedValue * 1.02), beds: listing.bedrooms ?? 3, baths: listing.bathrooms ?? 2, sqft: baseSqft + Math.floor(Math.random() * 200 - 100), soldDate: '2024-10-28', daysOnMarket: 8 },
      { address: '2134 Cedar Lane', distance: '0.7 mi', price: Math.floor(estimatedValue * 0.96), beds: (listing.bedrooms ?? 3) === 3 ? 4 : (listing.bedrooms ?? 3), baths: listing.bathrooms ?? 2, sqft: baseSqft + Math.floor(Math.random() * 300 - 150), soldDate: '2024-10-05', daysOnMarket: 15 },
    ];
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[14px]">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{listing.price ? `${listing.price.toLocaleString()}` : '--'}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {listing.agentWebsite && (
                      <a href={listing.agentWebsite} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-white/10 text-white px-3 py-1.5 rounded-lg text-[13px] hover:bg-white/20 transition-colors">
                        <Globe className="w-3.5 h-3.5" />Agent Website
                      </a>
                    )}
                  </div>
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                        ...
                        {listing.officeWebsite && (
                          <a href={listing.officeWebsite} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-white/70 text-[12px] hover:text-white">
                            <Globe className="w-3 h-3" />Office Website
                          </a>
                        )}
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[14px]">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-[18px] dark:text-white">{listing.sqft > 0 ? `$${Math.round(listing.price / listing.sqft).toLocaleString()}` : '--'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 mb-0.5">Status</p>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${listing.status === 'Active' ? 'bg-green-100 text-green-800' : listing.status === 'Pending' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-700'}`}>{listing.status}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium dark:text-white">{listing.listedDate ? new Date(listing.listedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '--'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 mb-0.5">Last Seen</p>
                      <p className="font-medium dark:text-white">{listing.lastSeenDate ? new Date(listing.lastSeenDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '--'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 mb-0.5">Listing Type</p>
                      <p className="font-medium dark:text-white">{listing.listingType || listing.listingTypeDetail || '--'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 mb-0.5">MLS Number</p>
                      <p className="font-medium font-mono text-[13px] dark:text-white">{listing.mlsNumber || '--'}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 mb-0.5">MLS Name</p>
                      <p className="font-medium text-[13px] dark:text-white">{listing.mlsName || '--'}</p>
                    </div>
                    {listing.removedDate && (
                      <div>
                        <p className="text-gray-500 mb-0.5">Removed Date</p>
                        <p className="font-medium dark:text-white">{new Date(listing.removedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                      </div>
                    )}
                    {listing.priceDrop && (
                    ...
                    {listing.virtualTourUrl && (
                      <div className="col-span-2">
                        <a href={listing.virtualTourUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[13px] text-[#342e37] dark:text-[#FFCE0A] font-medium hover:underline">
                          <Globe className="w-4 h-4" />Virtual Tour Available
                        </a>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[14px]">
                    ...
                    {listing.garage != null && (
                      <div>
                        ...
                        <p className="font-medium dark:text-white">{listing.garage ? `Yes${listing.garageSpaces ? ` (${listing.garageSpaces} spaces)` : ''}` : 'No'}</p>
                      </div>
                    )}
                    ...
                    {listing.hoaFee != null && (
                      <div><p className="text-gray-500 mb-0.5">HOA Fee</p><p className="font-medium dark:text-white">${listing.hoaFee?.toLocaleString()}/mo</p></div>
                    )}
                  </div>
                </div>
                        ...
                        .map(([date, event]: [string, any]) => (
                          <div key={date} className="flex items-start justify-between border border-gray-100 dark:border-white/10 rounded-lg px-3 py-2 text-[13px]">
                            <div>
                              ...
                              <p className="text-gray-500 text-[12px]">{event.event || '--'}{event.listingType ? ` - ${event.listingType}` : ''}</p>
                            </div>
                            <div className="text-right">
