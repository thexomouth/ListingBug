...
import { ArrowLeft, Search, MapPin, Calendar, Home, DollarSign, Download } from 'lucide-react';
...
export function SearchResultsPage({ searchRun, onBack }: SearchResultsPageProps) {
  ...
  const results = searchRun.results || [];
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-6 pb-12">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <ExportDropdown
            ...
            onSendToIntegration={(integration) => toast.success(`Sending ${results.length} listings to ${integration}...`)}
          />
        </div>

        {/* Search metadata pills */}
      ...
      {results.length === 0 ? (
        <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <p className="text-gray-500 font-medium">No listings were returned for this search</p>
          <p className="text-sm text-gray-400 mt-1">Try re-running the search with different criteria</p>
        </div>
      ) : (
        <>
          {/* Results table */}
